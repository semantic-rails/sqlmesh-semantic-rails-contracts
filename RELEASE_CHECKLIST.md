...
4. Replace the `compatibility.json` `engine_candidate_sha` placeholder with the
   ...
   Before publication, keep `engine.release_state` set to `candidate` so
   ordinary CI builds only that immutable revision.
5. Verify public engine tag `v0.2.0` dereferences to that exact commit and the
   matching engine artifact is on PyPI. Then change `engine.release_state` to
